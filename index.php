...
session_start();
	if(isset($_POST['ac'])){
		...
		$sql = "INSERT INTO GioHang(BookID, Quantity, Price, TotalPrice) VALUES('".$bookID."', ".$quantity.", ".$price.", Price * Quantity)";
		$conn->query($sql);
	}
...
echo '<blockquote>';
    ...
    while($row = $result->fetch_assoc()) {
	    ...
	    echo "<table>";
	   	...
	   	<input class="button" type="submit" value="Add to GioHang"/>
	   	</form></td></tr>';
	   	echo "</table>";
	   	echo "</td>";
    }
	...
	$sql = "SELECT book.BookTitle, book.Image, GioHang.Price, GioHang.Quantity, GioHang.TotalPrice FROM book,GioHang WHERE book.BookID = GioHang.BookID;";
	$result = $conn->query($sql);

    echo "<table style='width:20%; float:right;'>";
    echo "<th style='text-align:left;'><i class='fa fa-shopping-GioHang' style='font-size:24px'></i> GioHang <form style='float:right;' action='' method='post'><input type='hidden' name='delc'/><input class='cbtn' type='submit' value='Empty GioHang'></form></th>";
    $total = 0;
    while($row = $result->fetch_assoc()){
    	echo "<tr><td>";
