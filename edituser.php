<?php
session_start();
$nameErr = $emailErr = $genderErr = $addressErr = $icErr = $contactErr = $usernameErr = $passwBienLair = "";
...
$sql = "SELECT users.UserName, users.Password, ThanhVien.ThanhVienName, ThanhVien.ThanhVienEmail, ThanhVien.ThanhVienPhone, ThanhVien.ThanhVienGender, ThanhVien.ThanhVienAddress
	FROM users, ThanhVien
	WHERE users.UserID = ThanhVien.UserID AND users.UserID = ".$_SESSION['id']."";
$result = $conn->query($sql);
while($row = $result->fetch_assoc()){
	$oUserName = $row['UserName'];
	$oPassword = $row['Password'];
	$oName = $row['ThanhVienName'];
	$oIC = $row['ThanhVienIC'];
	$oEmail = $row['ThanhVienEmail'];
	$oPhone = $row['ThanhVienPhone'];
	$oAddress = $row['ThanhVienAddress'];
}
		...
		if (!preg_match("/^[a-zA-Z ]*$/", $name)){
			...
			if (empty($_POST["uname"])) {
				...
				if (empty($_POST["upassword"])) {
					$passwBienLair = "Please enter your Password";
					$upassword = "";
				}else{
										...
										if(!preg_match("/^[0-9 -]*$/", $contact)){
											...
											if (empty($_POST["gender"])){
												...
												if (empty($_POST["address"])){
													...
													$sql = "UPDATE ThanhVien SET ThanhVienName = '".$name."', ThanhVienPhone = '".$contact."', 
													 ThanhVienEmail = '".$email."', ThanhVienAddress = '".$address."', 
													ThanhVienGender = '".$gender."'";
													$conn->query($sql);
...
<form method="post"  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	...
	<span class="error" style="color: red; font-size: 0.8em;"><?php echo $passwBienLair;?></span><br><br>
